...
2. Initialize a variable `last_end` to track the ending time of the last selected movie. This ensures that no two selected movies overlap, as a movie can only be watched if its starting time is $\geq \texttt{last\_end}$.
3. Iterate through the sorted list of movies. For each movie, check if its starting time $a$ satisfies $a \geq \text{last\_end}$. If so, select the movie, increment the count of watched movies, and update `last_end` to the current movie's ending time $b$.
